# Dottie v6.5 LLMVM Redesign — From JSON Loop to Python Runtime
...
|---|----------|---------------------------|--------------|-----------|---------------|
| 1 | `dottie/pipeline/manifest.py` + `dedup.py` BEGIN IMMEDIATE | `grep -rn BEGIN IMMEDIATE` 5 hits, each claim requires 1 JSON call + sqlite lock. 2 procs contending = 2x round-trip per shard. 74 shards x 2 = 148 calls per expansion | Deadlock risk, backpressure 27 pending, sleep 2s loop | `asyncio.gather()` claims in one cell, persistent kernel holds DB handle | 148 calls → 1 cell, ~14k tokens → 900 tokens |
...
## Architecture Diagram — LLMVM for Dottie
...
## Self-Modification Examples — Real Code Dottie Writes

### Example 1: Agent adds caching layer to own download function

```python
# Initial state — from dottie/llmvm/tool_registry.py, no cache, 74 shards x download
download_cache = {}
...
from dottie.llmvm.self_modify import SelfModifyManager
...
def audit_jspace_leak():
    ...
    txt = Path("dottie/attention/compressed_conv.py").read_text()
    ...
    if "broadcast" in Path("dottie/model.py").read_text() and "future" in Path("dottie/model.py").read_text():
...
from dottie.llmvm.skillbook import SkillBookManager
...
# python -c "from dottie.llmvm.tmux import TmuxManager; m=TmuxManager(); m.create_session('train'); print(m.capture_pane('train', 50))"
...
if 'Backpressure: 27 shards pending > 20' in output:
    ...
    tmux.send_keys('train', 'python -m dottie.llmvm.kernel --exec "print(context.compact())"\n')
    ...
    tmux.send_keys('train', 'grep -A2 "RoPE" dottie/config.py\n')
else:
...
# from dottie.llmvm import LLMVMKernel, TmuxManager
# kernel = LLMVMKernel()
...
1. `jspace-inspector` → `dottie/skills/jspace-inspector/` — Planner hl150, S1 hl8 vs S2 hl300 selectivity, Critic early warning. Bootstrap: `python -m skills.loader run jspace-inspector --mode mock`
...
7. `eval-harness-runner` → harness mock instant no torch, real with `dottie_stable_736k.pt`
...
# python -c "from dottie.llmvm import LLMVMKernel; ..."
...
**Problem:** If agent has access to all tools from Confucius/DevMate/Metamate equivalent (for Dottie: HF, Ollama, W&B, GitHub, OpenWiki, code search, logs, datasets), doesn't that blow up context?

**No — defer-loaded:**

- **Upfront:** ~100 tokens per skill lightweight metadata. For Dottie's 11 skillbooks: 1,100 tokens. Lists name, 80-char desc, namespace.
  ```python
  from dottie.llmvm.tool_registry import create_ava_registry
...
```python
from dottie.llmvm import LLMVMKernel, ToolRegistry
kernel = LLMVMKernel()
reg = ToolRegistry()
...
- One cell: load `checkpoints/builder_state.json` + `manifest_20260716_115531.jsonl` + `dottie/config.py` RoPE schedule, pandas plot, save to `reports/report_real.html`
...
# parse RoPE schedule from dottie/config.py
...
- **Tool definition footer:** Every `dottie/llmvm/*.py` includes "Solo personal project, no connection to employer, built with public/free-tier only"
...
1. Merge `dottie/llmvm/` package — 6 files, ~25KB, free-tier, no work IP
...
4. Add 3 cron enhancements with LLMVM: dottie-data-gather parallel gather(), dottie-eval-distill with context compaction, dottie-tmux-watchdog for training stall
5. Push to prod: `https://github.com/jcdavis131/dottie-agi-factory-v6-4.git` — include `docs/prompts/METAMATE_AUTO_REVIEW_PROMPT.md` + this doc
...
**Refs:** Metamate Advanced Auto note 2026-07-15 (LLMVM, UnifiedCodeExtension, TMUX extension, Skills/Skillbooks, defer-load 1000+ tools), Anthropic tool use blog on advanced tool use, Claude Code sequential JSON vs asyncio.gather() parallel, Dottie v6.4 README + ORCHESTRATION.md + HARNESS_SKILL_INTEGRATION.md
