...
- **Worker tier:** Sonnet
- **Dependencies:** 01_environment (env + DottieConfig); 02_data_generation (needs raw JSONL in
...
1. `dottie/tokenizer.py` (module + CLI)
2. `tests/test_tokenizer.py`
3. Artifact produced by running the CLI: `data/nano/tokenizer/dottie_nano_bpe.json`
...
class DottieTokenizer:
    @classmethod
    def train(cls, preset: str, raw_dir: str | None = None, seed: int = 1234) -> "DottieTokenizer"
    @classmethod
    def load(cls, path: str | None = None, preset: str = "nano") -> "DottieTokenizer"  # default data/{preset}/tokenizer/ava_{preset}_bpe.json
...
- Vocab size is read from `DottieConfig.load(preset).vocab_size` — nano→8192; the code takes it
...
### CLI
- `python -m dottie.tokenizer train --preset nano [--raw-dir data/nano/raw] [--seed 1234]`
  → trains, writes artifact + meta, prints one JSON line
  `{"artifact": "data/nano/tokenizer/dottie_nano_bpe.json", "vocab_size": 8192, "sample_bytes": N, "compression_chars_per_token": X.XX}`
...
- `python -m dottie.tokenizer encode --text "spider has 8 legs"` → prints ids (debug convenience).
...
2MB of raw data into a tmp dir via the same code path; if `data/nano/tokenizer/dottie_nano_bpe.json`
...
- `test_load_speed`: `DottieTokenizer.load()` completes in < 1.0s (time.perf_counter).
...
- Artifact is a standard `tokenizers.Tokenizer.save()` JSON — loadable by
  `tokenizers.Tokenizer.from_file()` directly; `DottieTokenizer` is a thin wrapper, so future GPU-side
...
1. `python -m dottie.tokenizer train --preset nano` → exits 0 in <10 min;
   `data/nano/tokenizer/dottie_nano_bpe.json` and `train_meta.json` exist; printed
   `compression_chars_per_token >= 3.0` and `vocab_size == 8192`.
2. `python -c "from dottie.tokenizer import DottieTokenizer; t=DottieTokenizer.load(); ids=t.encode('<|user|>Is 17 prime?<|assistant|>'); assert ids[0]==4 and 5 in ids; assert t.decode(t.encode('araña ↔ 蜘蛛'))=='araña ↔ 蜘蛛'; print('ok', t.vocab_size)"`
   → prints `ok 8192`.
3. `python -c "import time,importlib; s=time.perf_counter(); from dottie.tokenizer import DottieTokenizer; DottieTokenizer.load(); print(time.perf_counter()-s)"` → < 1.0.
