...
Stand up the Python environment and package scaffolding for the real-mode Dottie build inside the
...
spec 04 only) and `server.py` (spec 07 only) — and `dottie/` code DOES import `model_1b.py` /
`multi_jspace_module.py` (they are the real model modules). All new code lives under `dottie/`,
`scripts/`, `tests/`, `configs/`.

## Deliverable files (exact paths, all repo-relative)

1. `scripts/setup_env.sh`
2. `dottie/__init__.py`
3. `dottie/config.py`
4. `configs/nano.yaml`
...
- `DottieConfig` is a typed dataclass tree mirroring those sections (e.g. `cfg.model.d_model`,
  `cfg.jspace.slots["system1"]`, `cfg.training.wsd.lr_max`, `cfg.phases[0].mix`).
- `DottieConfig.load(preset_name: str) -> DottieConfig` reads `configs/{preset_name}.yaml`
...
- CLI: `python -m dottie.config --preset nano --count-params` prints exactly one line:
...
Do not rewrite it; `DottieConfig` must parse it as-is (headline values: vocab 8192, d_model 256,
...
`DottieConfig.load` must parse them too (they add `milestones:`/`branches:`/`frac:` fields).
...
- `tokenizer` → `python -m dottie.tokenizer train --preset nano` (spec 03)
- `pack` → `python -m dottie.data.pack --preset nano` (future spec; stub target OK)
- `smoke` → `python -m dottie.train --preset nano_quick --max-steps 20` (future spec; stub target OK)
- `train-nano` → `python -m dottie.train --preset nano` (future)
- `eval` → `python -m dottie.eval --preset nano` (future)
- `serve` → `uvicorn server:app --host 0.0.0.0 --port 8000` (spec 07 rewires the root server.py)
- `report` → `python -m dottie.report --out reports/` (future)
- `test` → `pytest`
...
- `test_nano_loads`: `DottieConfig.load("nano")` returns the exact values above.
...
## Interfaces / schemas
- Downstream specs import `from dottie.config import DottieConfig` and call `DottieConfig.load(preset)`.
...
3. `python -m dottie.config --preset nano --count-params` → exits 0, printed param count between
   13,000,000 and 16,000,000.
4. `python -m dottie.config --preset nano_quick --count-params` → exits 0.
...
7. `git status --porcelain` shows only NEW files under dottie/, scripts/, configs/, tests/, specs/,
   pytest.ini, Makefile, plus the .gitignore append — no blueprint file modified.

## Out of scope
- Model architecture code (dottie/model.py), training loop, packing, eval, server — later specs.
