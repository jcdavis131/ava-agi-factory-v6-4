"""
CompressionGenerator — B6 synthetic dataset for Ava v6.4
...
"""
        ...
        if len(text) < 500:
            text += "\nExtra context: Shannon 1948 paper, redundancy, mutual information I(X;Y)=H(X)-H(X|Y), used in Ava's OroJaR Jacobian regularizer for orthogonal workspaces.\n" * 2
...
Additional long context for p4: DeepMind language modeling is compression: 12x compression ImageNet patches. Training over neurally compressed text: M1 compresses raw bytes via arithmetic coding, M2 trains over compressed bitstream. Finding AC-compressed not readily learnable -> need learned Z-tokens not pure gzip. This maps to Ava J-Space: 144 slots (S1 32 hl8 auto, S2 64 hl300 deliberate, Critic 16 hl30 safety, Planner 32 hl150) compressing 2k-131k at 20% broadcast target. Concept token France->Paris etc measures if same slot fires for paraphrases, see evals/probes.py Z-slot cosine.
...
"""
        ...
        if is_long and len(text) < 6000:
            ...
            extra += "\n\nFor Alienware 4080 12GB: base1b 1409M params bf16 2.8GB weights + 2.8GB grads + 1.4GB AdamW8bit = 8.4GB before activations. At 131k, KV 7.52GB -> DeltaNet 21+7 split 1.90GB + FP8 KV + W4A16 GPTQ for serving compression. This textbook is part of teaching Ava to reason about its own serving constraints.\n"
            text += extra
    ...
    def _gen_ztoken(self, idx: int) -> dict:
        """
        Z-token compressor/decompressor/inferencer paradigm — maps directly to Ava J-Space
        ...
        r = target compression ratio, e.g., 5 => 20% (Ava broadcast 0.20)
        """
        ...
        for i in range(para_len):
            ...
            subj = self.rng.choice(["The model", "Ava", "The system", "S2 workspace", "Planner"])
...
Goal: compress X to Z with ratio r={r} (20% like Ava broadcast 0.20). Target K = |X|/r ≈ {target_k} tokens.
...
Mapping to Ava J-Space (144 total slots):
...
Training over neurally compressed text: M1 compresses raw bytes via arithmetic coding, M2 trains over compressed bitstream chunked into tokens. Finding: AC-compressed text NOT readily learnable even with unigram M1 — suggests need learnable Z-tokens not pure gzip. Implication: don't use pure gzip for training, use learned slots. In Ava streaming_data.py, neural_compress option feeds through current checkpoint to compress then trains decompressor.

Eval metric: compression reconstruction BLEU/ROUGE + enwik9 bits per byte via CE: measure Ava cross-entropy on enwik9 as compression rate. Also Z-slot cosine similarity across paraphrases "France capital Paris" vs "Paris is capital of France" should share slot.
...
"""
        ...
        if is_long and len(text) < 6000:
            ...
            extra += "\nFor Ava training at scale: WSD 736k 92% stable + WSM merging infinite continuation, no LR decay collapse, matches modular manifold schedule. After stable 736k checkpoint, branch code/math/chat with 2-stage SFT + MaxEnt RL + offline self-distill $7.8k budget analog local. Compression textbook adds 30MB verified synthetic fits existing pipeline, no GPU needed, unblocks Phase0-2 quality.\n"
            text += extra
            while len(text) < 6000:
